...
def simulate(X_train, X_test, errors, s, models, lambda_qut, run_id):
    # Generate dataset
    features = pd.read_csv(f"parallelization/linear/data/features_{s}.csv")[f"{run_id}"]
    beta = 3 * np.ones(s)
    if features.empty:
...
if __name__ == "__main__":
    m = 100
    s_values = range(31)
    X_train = pd.read_csv("parallelization/linear/data/X_train.csv")
    X_test = pd.read_csv("parallelization/linear/data/X_test.csv")
    errors = pd.read_csv("parallelization/linear/data/errors.csv")
