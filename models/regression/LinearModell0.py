...
def FISTAsqrtLASSO(X, y, lambda_, tol=1.e-11):
    ...
    for i in range(6):
        ...
        cost = np.sqrt(np.sum((y - alpha_int - ypred)**2)) + lambdai * np.sum(np.abs(alpha) / (1 + np.abs(alpha)**(1 - 0.1)))
        costs = [cost]
        epoch = 0
        while continue1:
            continue2 = True
            ...
            while continue2:
                ...
                solj = utils.shrinkage_operator(torch.tensor(uj), torch.tensor(lambdaj), 0.1).numpy()
                ypredj = np.dot(X, solj)
                resj = y - ypredj
                alpha_intj = np.mean(resj)
                costnew = np.sqrt(np.sum((resj - alpha_intj)**2)) + lambdai * np.sum(np.abs(solj) / (1 + np.abs(solj)**(1 - 0.1)))
                if costnew > cost:
                    j += 1
                else:
                    continue2 = False
