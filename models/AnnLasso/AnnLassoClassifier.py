...
class AnnLassoClassifier(nn.Module):
    ...
    def _build_layers(self, input_dim, output_dim):
        ...
        for i in range(1, len(self.hidden_dims)+1):
            output = output_dim if i == len(self.hidden_dims) else self.hidden_dims[i]
    ...
    def fit(self, X, y, verbose=False):
        ...
        if self.lambda_qut is None:
            self.lambda_qut = utils.lambda_qut_classification(X, utils.get_hat_p(y), self.act_fun, self.hidden_dims)

        self._train_loop(X, y, verbose)
        self.imp_feat = self.important_features()

    def _train_loop(self, X, y, verbose):
        utils.sgd(self, X, y, 0.01, torch.tensor(0), 1, 1e-2, verbose, type='class')
        for i in range(6):
            ...
            if i<5:
                utils.sgd(self, X, y, lr, lambda_i, nu, rel_err, verbose, type='class')
            else:
                utils.training(self, X, y, lambda_i, nu, rel_err, verbose, type='class')
    ...
    def load(self, filepath, strict=True):
        ...
        output_dim = checkpoint['state_dict'][f'layers.{len(hidden_dims)}.weight'].shape[0]
        self._build_layers(input_dim, output_dim)
